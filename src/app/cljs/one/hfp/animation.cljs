(ns ^{:doc "Defines animations which are used in the hfp
  application."}
  one.hfp.animation
...
;;(def proj-list "projectArea") 

(def fade-in {:effect :fade :end 1 :time 500})

(def fade-out {:effect :fade :end 0 :time 500})

(defn p-list-show
  "Fades in project list"
  [proj-list]
  (set-styles! (single-node (by-id proj-list)) {:opacity "0.1" :display "block"})
  (let [div (by-id proj-list)]
    (play div fade-in)))
    
(defn p-list-hide
  "Fades out project list"
  [proj-list]
  (let [div (by-id proj-list)]
    (play div fade-out)))
